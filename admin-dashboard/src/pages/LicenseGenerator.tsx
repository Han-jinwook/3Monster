                    ...
                    <AnimatePresence>
                        {generatedKey && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                                <Card className={`${
                                    generatedKey.startsWith('TEST-') ? 'bg-emerald-600' : 'bg-indigo-600'
                                } text-white p-6 space-y-4 shadow-lg rounded-2xl border-none`}>
                                    <div className="flex items-center gap-3">
                                        <div className="h-9 w-9 rounded-xl bg-white/20 flex items-center justify-center">
                                            {generatedKey.startsWith('TEST-') ? <Clock className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
                                        </div>
                                        <h4 className="font-black text-sm">
                                            {generatedKey.startsWith('TEST-') ? '테스트 라이선스 키 발급 완료' :
                                             generatedKey.startsWith('TRIAL-') ? 'DELUXE (5일 체험판) 라이선스 발급 완료' :
                                             '정식 라이선스 발급 완료'}
                                        </h4>
                                    </div>
                                    <div className="rounded-xl bg-white/10 p-4 text-center space-y-1">
                                        <p className="font-mono text-base font-black tracking-wider">{generatedKey}</p>
                                        {generatedKey.startsWith('TRIAL-') && (
                                            <p className="text-[10px] text-indigo-200 font-bold">⚠️ 5일 / 500건 제한 — 구매자에게 시리얼 전달 필요</p>
                                        )}
                                    </div>
                                    <Button
